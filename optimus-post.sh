#!/bin/bash

case $1 in
  init_env)
    cat ./front/.env.example > ./front/.env
    cat ./api/.env.example > ./api/.env ;;
  init_db)
    echo "======== CREATE ROLE TO POSTGRES"
    docker exec -it optimus-post-postgres psql -h postgres -Upostgres -c "CREATE USER optimus_post WITH PASSWORD '$2';"
    docker exec -it optimus-post-postgres psql -h postgres -Upostgres -c "ALTER ROLE optimus_post SUPERUSER CREATEDB;"
    echo "======== MIGRATION DB"
    docker exec -it optimus-post-app bash -c "bundle install && bundle exec rails db:create db:migrate"
    echo "======== END" ;;
  start)
    docker-compose  --env-file ./api/.env -p optimus-post-api up -d ;;
  build)
    docker-compose  --env-file ./api/.env -p optimus-post-api up -d --build;;
  stop)
    docker-compose -p optimus-post-api down ;;
  dump)
    pg_dump -Oxc -h localhost -w -p 5432 -U optimus_post optimus_post_db > ./dump.sql ;;
  rmi)
    docker-compose -p optimus-post-api down --volumes
    docker-compose -p optimus-post-api down --rmi -f all
    docker rmi -f $(docker images -a -q) ;;
  console)
    docker exec -it optimus-post-app bash -c "bundle exec rails c" ;;
  connect)
    docker exec -it optimus-post-app bash ;;
  seed)
    docker exec -it optimus-post-app bash -c "bundle exec rails db:seed" ;;
  migrate)
    docker exec -it optimus-post-app bash -c "bundle exec rails db:migrate" ;;
  rspec)
    docker exec -it optimus-post-app bash -c "RAILS_ENV='test' bundle exec rspec" ;;
  swagger)
    docker exec -it optimus-post-app bash -c "SWAGGER_DRY_RUN=0 RAILS_ENV=test rake rswag:specs:swaggerize" ;;
  *)
    echo "Using: ./optimus-post.sh [init_db 'db_password', init_env, start, stop, rmi, build, connect, seed, console, migrate, rspec, swagger]"
    echo "./optimus-post.sh rmi removes all images in the system, be careful!" ;;
esac;
